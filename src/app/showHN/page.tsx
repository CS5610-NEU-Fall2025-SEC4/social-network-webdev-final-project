...
export default async function ShowHN() {
  try {
    ...
    if (!stories || !stories.hits || stories.hits.length === 0) {
      return (
        <div className="text-center py-12">
          <p className="text-gray-500 text-lg">No Show HN posts found.</p>
          <p className="text-gray-400 text-sm mt-2">Be the first to share your project!</p>
        </div>
      )
    }
    ...
    console.error('Failed to fetch Show HN posts:', error)
    return (
      <div className="text-center py-12">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 max-w-md mx-auto">
          <p className="text-red-700 font-medium text-lg mb-2">Failed to load Show HN posts</p>
